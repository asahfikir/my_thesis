...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Judge Model:} Qwen2.5-72B-Instruct (digunakan untuk evaluasi RAGAS).
...
\begin{itemize}
  ...
  \item Metrik End-to-End RAG (Generasi)\par
    ...
    Dalam penelitian ini, digunakan \textbf{Qwen2.5-72B-Instruct} sebagai judge model untuk mengevaluasi jawaban dari ketiga SLM. Pemilihan ini didasarkan pada pertimbangan bahwa model dengan 72 miliar parameter memiliki kapasitas penalaran yang jauh lebih superior dibandingkan SLM 3B yang dievaluasi, namun tetap dalam satu keluarga model (Qwen2.5) sehingga memiliki konsistensi dalam pemahaman bahasa Indonesia. Penggunaan model yang lebih besar sebagai judge memastikan penilaian yang lebih akurat dan mengurangi risiko \textit{self-bias} yang terjadi jika model menilai jawabannya sendiri \citep{zheng2023judging}.
...
\begin{itemize}
    ...
    \item Koleksi Hasil dan Perhitungan Metrik: Hasil jawaban dan konteks yang diambil disimpan. Selanjutnya, metrik evaluasi (\textit{Recall}, \textit{F1}, \textit{Faithfulness}, dll.) dihitung. Untuk metrik \textit{RAGAS}, digunakan model \textbf{Qwen2.5-72B-Instruct} sebagai \textit{judge} otomatis untuk menilai kualitas jawaban tanpa bias terhadap model \textit{SLM} yang sedang diuji. Judge model dijalankan melalui API atau platform yang mendukung model berukuran besar (seperti Together AI, Fireworks, atau OpenRouter) untuk memastikan konsistensi evaluasi. Setiap jawaban dinilai berdasarkan faithfulness (apakah jawaban didukung konteks?), answer relevancy (apakah jawaban menjawab pertanyaan?), dan context precision/recall (seberapa relevan chunk yang diambil).
\end{itemize}
